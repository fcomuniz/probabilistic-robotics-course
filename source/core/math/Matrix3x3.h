...
public:
	...
	Matrix3x3<V>(const V& a11, const V& a12, const V& a13, const V& a21,
			const V& a22, const V& a23, const V& a31, const V& a32,
			const V& a33) {
		c[0].x = a11;
		c[1].x = a12;
		c[2].x = a13;
	...
	Matrix3x3<V> operator+(const Matrix3x3<V>& other) const {
		return Matrix3x3<V>((*this).c[0] + other.c[0],
				(*this).c[1] + other.c[1], (*this).c[2] + other.c[2]);
	}
	/**
...
public:
	 ...
	 * @param c2 The third column of the matrix.
	 */
	Matrix2x3<V>(const Vector2<V>& c0, const Vector2<V>& c1,
			const Vector2<V>& c2) :
			c0(c0), c1(c1), c2(c2) {
	}
	...
	Matrix2x3<V> operator*(const Matrix3x3<V>& matrix) const {
		return Matrix2x3<V>(*this * matrix.c[0], *this * matrix.c[1],
				*this * matrix.c[2]);
	}
	...
	Matrix3x2<V> transpose() {
		return Matrix2x3<V>(Vector2<V>(c0.x, c1.x, c2.x),
				Vector2<V>(c0.y, c1.y, c2.y));
	}
};
...
public:
	...
	Matrix2x3<V> transpose() {
		return Matrix2x3<V>(Vector2<V>(c0.x, c1.x), Vector2<V>(c0.y, c1.y),
				Vector2<V>(c0.z, c1.z));
	}
};
